...
export default function Navbar() {
  ...
  return (
    <>
      <header
        className={cn(
          "absolute top-0 left-0 right-0 z-[60] py-4 sm:py-6 transition-all duration-300 border-b bg-black/95 backdrop-blur-md border-white/10 shadow-sm"
        )}
      >
          ...
          <div className="flex items-center gap-3 sm:gap-6 z-50">
            ...
            <div className="relative" ref={currencyRef}>
              <button
                ...
                className="flex items-center gap-1.5 px-3 py-1.5 rounded-full text-xs sm:text-sm font-bold tracking-wider uppercase transition-all duration-200 border focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-white/40 text-white border-white/20 bg-white/10 hover:bg-white/20"
              >
                <span>{currency}</span>
              ...
              <AnimatePresence>
                {currencyOpen && (
                  <motion.div
                    ...
                    role="listbox"
                    className="absolute right-0 top-full mt-2 bg-[#141414] text-white rounded-2xl shadow-2xl border border-white/15 overflow-hidden z-[200] min-w-[120px] py-1.5"
                  >
                    {CURRENCIES.map((c) => (
                      <button
                        key={c}
                      ...
                      >
                        <span>{c}</span>
                        {currency === c && <span className="w-2 h-2 rounded-full bg-gold"></span>}
                      </button>
                    ))}
                  </motion.div>
                )}
            ...
            <button
              ...
              className="p-2 rounded-full transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-white/40 text-white hover:bg-white/10"
            ...
            >
              {isOpen ? (
                ...
                <div className="w-6 h-5 flex flex-col justify-between items-end">
                  <span className="w-full h-0.5 rounded-full bg-white transition-transform duration-300" />
                  <span className="w-4/5 h-0.5 rounded-full bg-white transition-transform duration-300" />
                  <span className="w-3/5 h-0.5 rounded-full bg-white transition-transform duration-300" />
                </div>
              )}
            </button>
          </div>
      ...
      <AnimatePresence>
        {isOpen && (
          <motion.div
            ...
            className="fixed inset-0 z-[50] w-full h-dvh bg-black/98 backdrop-blur-2xl text-white flex flex-col justify-between overflow-hidden pt-16 sm:pt-24 pb-6 px-6 sm:px-12 lg:px-20 xl:px-28"
          >
