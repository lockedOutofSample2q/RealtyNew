    ...
    <>
      <header
        className={cn(
          ...
          useDarkText ? "bg-white/95 backdrop-blur-md border-black/5 shadow-sm" : "bg-transparent border-transparent"
        )}
      >
        <div className="w-[92%] max-w-7xl mx-auto flex items-center justify-between">
          ...
          <Link href="/" onClick={() => setIsOpen(false)} className="flex items-center gap-2 group z-50">
            <div className="relative h-9 sm:h-12 w-32 sm:w-48 aspect-[1756/925]">
              <Image 
              ...
              <AnimatePresence>
                {currencyOpen && (
                  <motion.div
                    ...
                    role="listbox"
                    className="absolute right-0 top-full mt-2 bg-white text-charcoal rounded-2xl shadow-2xl border border-black/10 overflow-hidden z-[200] min-w-[120px] py-1.5"
                  >
                    {CURRENCIES.map((c) => (
                      <button
                        key={c}
                      ...
                      >
                        <span>{c}</span>
                        {currency === c && <span className="w-2 h-2 rounded-full bg-charcoal"></span>}
                      </button>
                    ))}
                  </motion.div>
                )}
      ...
      <AnimatePresence>
        {isOpen && (
          <motion.div
            ...
            className="fixed inset-0 z-[50] w-full h-dvh bg-charcoal/98 backdrop-blur-2xl text-white flex flex-col justify-between overflow-hidden pt-16 sm:pt-24 pb-6 px-6 sm:px-12 lg:px-20 xl:px-28"
          >
