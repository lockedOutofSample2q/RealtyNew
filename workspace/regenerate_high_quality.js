...
// Hand-picked verified regional pool
const verifiedPool = ['mohali_lux_1.jpg', 'mohali_lux_2.jpg'];
...
async function generateCover(outPath, title, category, seed) {
  const bgName = verifiedPool[seed % verifiedPool.length];
...
async function run() {
  ...
  console.log(`Generating visual transformation with 100% verified regional assets for ${blogs.length} blogs...`);
    ...
    try {
      ...
      if (fs.existsSync(mdxPath)) {
        ...
        for (let i = 0; i < plan.inlineImages.length; i++) {
          await generateInline(plan.inlineImages[i].path, seed + i);
        }
        count++;
  ...
  console.log('\nSuccess: Premium regional visuals generated.');
}
