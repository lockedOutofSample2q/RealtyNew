...
// Fallback pool based on the ONLY verified localized high-res asset
const sourcePool = ['ind_lux_final.jpg'];
...
async function generateCover(outPath, title, category, seed) {
    const bgPath = path.join(sourceDir, sourcePool[0]);
...
async function run() {
    ...
    console.log(`Generating visual assets for ${blogs.length} blogs using THE VERIFIED LOCALIZED ASSET...`);
    ...
    for (const id of blogs) {
        ...
        try {
            ...
            if (fs.existsSync(mdxPath)) {
                ...
                await generateCover(plan.coverImage, data.title || 'Untitled', data.category || 'Real Estate', parseInt(id));
