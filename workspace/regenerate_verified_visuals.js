...
const mapping = {
  'Sector Intelligence': 'ind_infra_1.jpg',
  'Land Seller Series': 'ind_land_1.jpg',
  'Buyer Guide': 'ind_lux_1.jpg',
  'Investment Thesis': 'ind_lux_1.jpg',
  'Legal Updates': 'ind_work_1.jpg',
  'Process & Paperwork': 'ind_work_1.jpg',
  'Market Analysis': 'ind_infra_1.jpg',
  'NRI Investor Series': 'ind_lux_1.jpg',
  'AEO Anchor & Authority Builders': 'ind_lux_1.jpg',
  'Buyer Protection': 'ind_work_1.jpg',
  'Foundation & Master Reference': 'ind_lux_1.jpg',
  'Investment Tips': 'ind_work_1.jpg',
  'Developer Due Diligence': 'ind_lux_1.jpg',
  'default': 'ind_lux_1.jpg'
};

const allAssets = ['ind_lux_1.jpg', 'ind_land_1.jpg', 'ind_infra_1.jpg', 'ind_work_1.jpg'];
...
async function generateCover(outPath, title, category) {
  const bgName = mapping[category] || mapping.default;
...
async function generateInline(outPath, seed) {
  const bgName = allAssets[seed % allAssets.length];
  const bgPath = path.join(sourceDir, bgName);
  const finalPath = path.join(publicDir, outPath.startsWith('/') ? outPath.slice(1) : outPath);
  const dir = path.dirname(finalPath);
  if (!fs.existsSync(dir)) fs.mkdirSync(dir, { recursive: true });
...
async function run() {
  ...
  console.log(`Regenerating verified visual assets for ${blogs.length} blogs...`);
    ...
    try {
      ...
      if (fs.existsSync(mdxPath)) {
        ...
        for (let i = 0; i < plan.inlineImages.length; i++) {
          await generateInline(plan.inlineImages[i].path, parseInt(id) + i);
        }
  ...
  console.log('\nSuccess: Verified visuals generated.');
}
