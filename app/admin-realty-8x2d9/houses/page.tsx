  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className="font-display text-2xl text-white font-light">Houses & Villas</h1>
          <p className="font-body text-xs text-white/40 mt-1">{houses.length} total listings</p>
        </div>
        <button
        ...
        <table className="w-full">
          ...
          <tbody>
            {loading ? (
              ...
              <tr><td colSpan={5} className="px-5 py-10 text-center text-white/20">No houses found</td></tr>
            ) : (
              filtered.map((p) => (
                <tr key={p.id} className="border-b border-white/5 hover:bg-white/[0.02]">
                  ...
                  <td className="px-5 py-3">
                    <div className="flex items-center gap-2">
                      <button onClick={() => router.push(`/admin-realty-8x2d9/houses/${p.id}`)} className="p-1 text-white/40 hover:text-white transition-colors">
                        <Pencil size={14} />
                      </button>
