"use client";
// app/admin-realty-8x2d9/houses/page.tsx
// ============================================================
// HOUSES CMS (Villas/Townhouses)
// ============================================================
...
export default function HousesAdmin() {
  ...
  async function handleDelete(id: string) {
    if (!confirm("Delete this house listing?")) return;
    ...
    <div className="max-w-7xl mx-auto">
      <div className="flex items-center justify-between mb-8">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="font-display text-2xl font-semibold text-gray-900">Houses & Villas</h1>
            <p className="font-body text-sm text-gray-500 mt-1">{houses.length} total listings</p>
          </div>
        </div>
        <button
          onClick={() => router.push("/admin-realty-8x2d9/houses/new")}
          ...
          <table className="w-full text-left border-collapse">
            ...
            <tbody className="divide-y divide-gray-100">
              {loading ? (
                ...
                <tr><td colSpan={5} className="px-6 py-12 text-center text-gray-400 font-body text-sm">No houses found</td></tr>
              ) : (
                filtered.map((p) => (
                  <tr key={p.id} className="hover:bg-gray-50/50 transition-colors group">
                    ...
                    <td className="px-6 py-4">
                      <div className="flex items-center gap-3">
                        <button onClick={() => router.push(`/admin-realty-8x2d9/houses/${p.id}`)} className="p-1.5 text-gray-400 hover:text-blue-600 hover:bg-blue-50 rounded-md transition-colors" title="Edit">
                          <Pencil size={16} />
                        </button>
