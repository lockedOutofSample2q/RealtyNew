...
files.forEach(file => {
  ...
  // Replace generateMetadata entirely
  content = content.replace(/export async function generateMetadata.*?return \{[\s\S]*?\};\n\}/g, (match) => {
    return `export async function generateMetadata(props: Props): Promise<Metadata> {
  const params = await props.params;
  const p = await getProperty(params.slug);
  if (!p) return {};
  
  const cleanTitle = p.title.replace(/\\s*\\|\\s*Monte Real Estate/gi, '');
